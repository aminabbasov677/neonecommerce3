...
const Delivery = ({ stage, progress, origin, destination }) => {
  ...
  // Calculate position based on stage and progress
  const getPosition = () => {
    ...
    switch(stage) {
      ...
      case 'transit':
        const transitProgress = (progress - 30) / 40; // 30s warehouse, 40s transit
        return [
      ...
      case 'delivery':
        const deliveryProgress = (progress - 70) / 5; // 70s total before delivery
        return [
          destPos[0],
  ...
  return React.createElement(Box, { ref: packageRef, args: [0.5, 0.5, 0.5] },
